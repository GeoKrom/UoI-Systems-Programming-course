...
student_t *create_struct_list(char *name[], int am[], float grade[]){
...
{
	printf("The students that failed are:\n");
...
void change_failed_names(student_t *list)
{
	...
	while (list != NULL)        /* Until there is no other node in the list */
	{
		if(list->grade < 10)
		{
			...
			list->name = (char *) realloc(list->name, (strlen(list->name) + 10)*sizeof(char));
			if (list->name == NULL) exit(1);  
			strcat(list->name, " (failed)"); /* Concatenate the (failed) */
		}
		list = list->next;               /* Go to the next node */
	}
}
